...
def levelprint(pString, pLevel, pRemoteStamp, pApplicationName, pLocation, pApplicationCounter, pSourceName, pSourceTS ):
    ...
    msg = CLLOCDATETIMEFMT.format(str(msgNowString)) \
          + CLLEVELFMT.format(level) \
          + CLREMDATETIMEFMT.format(pRemoteStamp) \
          + CLAPPNAMEFMT.format(pApplicationName) \
          + CLREMDATETIMEFMT.format("" if (pSourceTS == pRemoteStamp) else pSourceTS) \
          + CLAPPNAMEFMT.format("" if (pSourceName == pApplicationName) else pSourceName) \
          + CLAPPLOCFMT.format(pLocation) \
          + CLAPPCNTFMT.format(pApplicationCounter) \
          + CLLOCCNTFMT.format(localCounter) \
          + "  " \
          + pString
...
def PrintNetworkMessage(messageParts):
    ...
    if len(messageParts) == 9:
        try:
            ...
            appName2 = messageParts[7].decode('utf-8')
            appTS = messageParts[8].decode('utf-8')
            # for eachString in splittedString:
            levelprint(logString,
                       levelValue,
                       remoteStamp,
                       appName,
                       appLocation,
        ...
        if len(messageParts) == 7:
            try:
                ...
                appTS = "?"
                # for eachString in splittedString:
                levelprint(logString, levelValue, remoteStamp, appName, appLocation, appCounter, appName2, appTS)
            except Exception as ex:
                levelprint(str(ex), logging.ERROR, "", APPLOGNAME, "localhost", localCounter,"","")
        else:
...
# 06: Level                            I,E,D
# 07: Stamp der Anwendung, wenn der Event über die DB gekommen ist (LOGV2)
# 08: Name der Anwendung, wenn der Event über die DB gekommen ist (LOGV2)
# Die Ausgabe erfolgt:
#
